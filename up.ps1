...
dotnet sitecore login --cm https://cm.sitecore_docker_nextjs.localhost/ --auth https://id.sitecore_docker_nextjs.localhost/ --allow-write true
if ($LASTEXITCODE -ne 0) {
...
Invoke-RestMethod "https://cm.sitecore_docker_nextjs.localhost/sitecore/admin/PopulateManagedSchema.aspx?indexes=all" -Headers @{Authorization = "Bearer $token"} -UseBasicParsing | Out-Null
...
Start-Process https://cm.sitecore_docker_nextjs.localhost/sitecore/
Start-Process https://www.sitecore_docker_nextjs.localhost/
